...
export function useSheltersPublic({ page = 1, pageSize = 10, fetchAll = false } = {}) {
  ...
  const fetchPage = useCallback(async () => {
    ...
    try {
      const base = supabase
        .from('shelters')
        .select('id, slug, name, city, province, address, lat, lng, is_active, shelter_config(shelter_image_url, province), pets(adoption_status), volunteer_subscriptions(count)', { count: 'exact' })
        .eq('is_active', true)
